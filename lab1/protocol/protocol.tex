...
Dodatkowo podczas drugiej sesji laboratoryjnej dodany skrypt w Ruby został poszerzony o możliwość wysyłania co 10 sekund pakietów UDP z obecną datą i godziną na maszynę laboratoryjną.
...
\end{lstlisting}
Dodatkowo, \emph{init script} został zmodyfikowany tak, aby daemon \verb+ntpd+ skanował zmiany na interfejsach co 5 sekund, poprzez dodanie opcji \verb+-U5+:
...
Po wykonaniu powyższych czynności pozwala na zainstalowanie paczki w katalogu \verb+/usr/bin+ systemu wynikowego. Jedyne, co pozostało zrobić, to dodanie \emph{init scriptu} uruchamiającego \verb+hello.rb+ w tle do katalogu \verb+/etc/init.d/+ nakładki:
